...
class ConverterVCD400(Converter):
    # This class receives a VCD 4.0.0, and creates a VCD 3.3.0 file
    # Creates a Frame-wise VCD 3.3.0 JSON file
    def __init__(self, vcd400_file_name):
        Converter.__init__(self, vcd400_file_name)

        # Load JSON file with VCD 4.0.0 content
        vcd400 = core.VCD(vcd400_file_name, validation=True)
        ...
        # Root and metadata
        self.vcd['VCD'] = {
            'annotator': vcd400.get_metadata()['annotator'],
            'name': vcd400.data['vcd']['name'],
            'metaData': {}
        }
        if 'comment' in vcd400.get_metadata():
            self.vcd['VCD']['scdName'] = vcd400.get_metadata()['comment']
        if 'streams' in vcd400.get_metadata():
            for stream in vcd400.get_metadata()['streams'].keys():
                self.vcd['VCD']['metaData'].setdefault('stream', [])
                if 'stream_properties' in vcd400.get_metadata()['streams'][stream]:
                    self.vcd['VCD']['metaData']['stream'].append(
                        {"description": vcd400.get_metadata()['streams'][stream]['description'],
                         "uri": vcd400.get_metadata()['streams'][stream]['uri'],
                         "name": stream,
                         "type": vcd400.get_metadata()['streams'][stream]['type'],
                         "streamProperties": vcd400.get_metadata()['streams'][stream]['stream_properties']}
                    )
                else:
                    self.vcd['VCD']['metaData']['stream'].append(
                        {"description": vcd400.get_metadata()['streams'][stream]['description'],
        ...
        # Frame intervals
        fis = vcd400.data['vcd']['frame_intervals']
        frameStart = 0
        frameEnd = 0
        for fi in fis:
        ...
        # Ontologies
        if 'ontologies' in vcd400.data['vcd']:
            self.vcd['VCD']['OntologyManager'] = {}
            self.vcd['VCD']['OntologyManager']['ontology'] = []
            for ontology_uid in vcd400.data['vcd']['ontologies'].keys():
                self.vcd['VCD']['OntologyManager']['ontology'].append(
                    vcd400.data['vcd']['ontologies'][ontology_uid]
                )

        # StaticAttributes (objects and potentially contexts if no frame interval inside)
        if 'objects' in vcd400.data['vcd']:
            for object_key in vcd400.data['vcd']['objects'].keys():
                object = vcd400.data['vcd']['objects'][object_key]
        ...
        if 'contexts' in vcd400.data['vcd']:
            for context_key in vcd400.data['vcd']['contexts'].keys():
                context4 = vcd400.data['vcd']['contexts'][context_key]
        ...
        for frame_num in vcd400.data['vcd']['frames'].keys():
            frame4 = vcd400.data['vcd']['frames'][frame_num]
            ...
            for elements in frame4.keys():
                if elements == 'frame_properties':
                    ...
                    for element4_id in elements4:
                        ...
                        element4_static = vcd400.data['vcd'][elements][element4_id]
                        obj = {}
    ...
    def __add_attributes(self, src400, dst330):
        if 'attributes' in src400:
            attrib3_dict = {}
            attrib3_type = ""
            for attrib4_key in src400['attributes'].keys():
                ...
                for it4 in src400['attributes'][attrib4_key]:
                    it3 = {}
                    ...
                    if attrib4_key == "num":
                        ...
                        if 'val' in it4:
                            it3.update({"val": it4['val']})
                    if "stream" in it4:
                        warnings.warn("WARNING: VCD4.0.0 allows stream info for "
            ...
            for object_data_item in root['object_data'][object_data_key]:
                ...
                if object_data_key == "bbox":
                    ...
                    for key, val in object_data_item['area_reference'].items():
                        ref_type400 = val['reference_type']
                        if ref_type400 == "line_reference":
                            refType330 = "lineReference"
                        elif ref_type400 == "point3d":
                            refType330 = "point3D"
                        else:
                            refType330 = ref_type400
                        if 'val' in val:

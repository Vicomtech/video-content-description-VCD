...
SCENARIO("Create some basic content, without time information") {
    GIVEN("A VCD structure") {
        THEN("Test number based uid") {
            ...
            const auto uid0 = vcd->add_object("Mike", mike_args);
            REQUIRE(uid0 == "0");
            ...
            susan_args.uid = "2";
            const obj_uid uid2 = vcd->add_object("Susan", susan_args);
            REQUIRE(uid2 == "2");

            // The user must use uuids in string format for all public functions
            vcd->add_bool_to_object(uid2, "checked", true);
            vcd->add_bool_to_object("2", "double-checked", true);

            // Same happens with ontology uids
            const vcd::ont_uid ont_uid_0 = vcd->add_ontology(
            ...
            const obj_uid uid3 = vcd->add_object("Mark", mark_args);
            const json* data = vcd->get_element(vcd::object, uid3);
        ...
        THEN("Test guid based uid") {
            ...
            const std::string test_uuid = vcd::VCD_UID::generate_uuid4();
            ...
            vcd::element_args susan_args;
            susan_args.semantic_type = "Person";
            const std::string uuid2 = vcd::VCD_UID::generate_uuid4();
        ...
        THEN("Use explicit uuid v4 values as input") {
            // 1.- Create a VCD instance
            vcd::VCD_ptr vcd_ptr = VCD::create();
            const std::string uuid1 = vcd::VCD_UID::generate_uuid4();
            ...
            std::string uid1 = vcd_ptr->add_object("marcos", marcos_args);
            const auto *object = vcd_ptr->get_object(uid1);
            REQUIRE(object != nullptr);
            REQUIRE((*object)["name"] == "marcos");

            vcd::element_args orti_args;
            orti_args.semantic_type = "person";
            const auto uid2 = vcd_ptr->add_object("orti", orti_args);

            const bool match_regex = vcd::VCD_UID::check_uuid4(uid2);
            REQUIRE(match_regex);
        }
    }
}
